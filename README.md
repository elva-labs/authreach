...
- **Connect any IMAP inbox** (iCloud, Fastmail, Yahoo, GMX, self-hosted, Proton via Bridge…) over TLS. Credentials live in your Keychain; the mailbox is opened read-only (`EXAMINE` + `BODY.PEEK`), so nothing is ever marked as read.
